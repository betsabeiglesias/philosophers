...
/*   Updated: 2024/12/24 18:57:10 by binary           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_philo(t_data *info)
{
	...
	while (i < info->nb_philo)
	{
		...
		info->phil[i].last_meal = info->time_start;
...
int	init_threads(t_data *info)
{
	...
	while (i < info->nb_philo)
	{
		...
		printf("Hilo creado para philo %d diferencia de tiempo %ld\n", info->phil[i].id, (info->phil[i].last_meal - get_time()));
		i++;
	}	
	return (EXIT_SUCCESS);
}
